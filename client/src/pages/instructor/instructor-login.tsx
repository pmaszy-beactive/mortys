...
import { Loader2, Users, AlertCircle, Sparkles, Award, Home } from "lucide-react";
...
export default function InstructorLogin() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      const response = await apiRequest("POST", "/api/instructor/login", data);
      
      if (response.success) {
        await queryClient.invalidateQueries({ queryKey: ["/api/instructor/me"] });
        
        toast({
          title: "Login successful",
          description: `Welcome back, ${response.instructor.firstName}!`,
        });
          ...
          <CardContent>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          type="email"
                          placeholder="instructor@mortys.com"
            ...
            <div className="mt-6 p-4 bg-gradient-to-r from-amber-50 to-yellow-50 rounded-xl border border-[#ECC462]/30">
              ...
              <div className="space-y-1 text-sm text-amber-800">
                <p><strong>Email:</strong> demo.instructor@example.com</p>
