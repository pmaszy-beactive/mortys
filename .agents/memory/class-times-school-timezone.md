...
Also: when `getClassStartTime` returns null (malformed schedule), fail closed for actions/policy (reject with a schedule-data error) but keep the class visible in read views — never fall back to a legacy server-local parse, since Invalid Date comparisons are always false and silently bypass gates.
